...
/**
 * @brief Compensa a pressão usando as fórmulas exatas do datasheet BMP180.
 */
static int32_t compensate_pressure(bmp180_dev_t *dev, int32_t up)
{
    ...
    // Datasheet formula exactly as specified
    b6 = dev->b5_comp - 4000;
    ...
    x3 = ((x1 + x2) + 2) >> 2;
    b4 = ((uint32_t)dev->calib_params.ac4 * (uint32_t)(x3 + 32768)) >> 15;

    // Calculate B7
    // CORREÇÃO: Cast explícito para garantir a subtração correta em unsigned
    // Datasheet Pág 15: B7 = ((unsigned long)UP - B3) * ...
    b7 = ((uint32_t)up - (uint32_t)b3) * (50000 >> BMP180_OSS);
    // Calculate p
    if (b7 < 0x80000000)
    {
        p = (b7 << 1) / b4;
...
static esp_err_t bmp180_read_calib_params(i2c_master_dev_handle_t dev_handle, bmp180_calib_param_t *params)
{
    ...
    if (ret == ESP_OK)
    {
        ...
        ESP_LOGI(TAG, "Calibração lida:");
        ESP_LOGI(TAG, "AC1=%d AC2=%d AC3=%d", params->ac1, params->ac2, params->ac3);
        ESP_LOGI(TAG, "AC4=%u AC5=%u AC6=%u", params->ac4, params->ac5, params->ac6);
        ESP_LOGI(TAG, "B1=%d B2=%d", params->b1, params->b2);
        ESP_LOGI(TAG, "MB=%d MC=%d MD=%d", params->mb, params->mc, params->md);
    }
    else
    {
...
esp_err_t bmp180_read_temperature(bmp180_dev_t *dev, int32_t *temperature)
{
    ...
    // Adiciona +1 tick para garantir que o delay não seja 0 se o tick rate for baixo (ex: 100Hz)
...
esp_err_t bmp180_read_pressure(bmp180_dev_t *dev, int32_t *pressure)
{
    if (!dev || !pressure)
        return ESP_ERR_INVALID_ARG;
    int32_t dummy_temp;
    // Precisamos ler a temperatura primeiro para atualizar b5_comp
    esp_err_t ret = bmp180_read_temperature(dev, &dummy_temp);
    if (ret != ESP_OK)
        return ret;
    ...
    // 2. Espera o tempo de conversão (depende do OSS)
    // OSS=0: 4.5ms, OSS=1: 7.5ms, OSS=2: 13.5ms, OSS=3: 25.5ms
    // Fórmula aproximada segura: 5 + (3 << OSS) -> 8, 11, 17, 29 ms
    uint32_t delay_ms = 5 + (3 << BMP180_OSS);

    // Adiciona +1 tick para garantir espera mínima
